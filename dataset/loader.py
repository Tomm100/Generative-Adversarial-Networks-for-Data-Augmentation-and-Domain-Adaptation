...
def setup_dataset(dataset_dir='./data/modified_dataset'):
    """Carica il modified_dataset (cartella o zip)."""
    ...
    if os.path.isdir(dataset_dir):
        ...
        print(f"ERRORE: ne {dataset_dir} ne {zip_path} trovati.")
        return None
...
def get_dataloaders(train_dir, val_dir, test_dir, img_size=128, batch_size=16):
    """Restituisce i DataLoader per ResNet (RGB, normalizzazione ImageNet)."""
...
def get_gan_dataloader(train_dir, img_size=128, batch_size=64):
    """Restituisce il DataLoader per il GAN (1 canale Grayscale)."""
    ...
    print(f"  GAN dataloader: {n_per_class} — shuffle=True")
