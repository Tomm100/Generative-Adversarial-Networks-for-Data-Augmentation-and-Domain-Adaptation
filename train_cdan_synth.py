"""
Training loop per CDAN Synthetic Domain Adaptation (Synth→Real).

Differenze rispetto al training DANN:
  - domain loss condizionata su f ⊗ softmax(g) (multilinear map)
  - Entropy Conditioning (CDAN+E): pesa la domain loss per entropia predizioni
  - il resto (task loss, λ scheduling, optimizer, model selection) è identico
...
def train_cdan_synth(
    ...
    use_entropy=True,
    tag="CDAN_Synth",
    checkpoints_dir="./results/cdan_synth_checkpoints",
    val_loader=None,
    class_names=None
):
    """
    Training loop CDAN Synth→Real (Supervised Domain Adaptation).

    Args:
        model:          CDANSynth instance
        ...
        use_entropy:    se True, applica Entropy Conditioning (CDAN+E)
    ...
    criterion_domain = nn.BCEWithLogitsLoss(reduction='none')  # reduction='none' per CDAN+E
    ...
    mode_str = "CDAN+E" if use_entropy else "CDAN"
    print(f"\n{'='*60}")
    print(f"  {mode_str} Synth→Real Training (Supervised DA) — {epochs} epoche")
    ...
    print(f"  Entropy Conditioning: {use_entropy}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in pbar:
            ...
            class_out_real, domain_out_real, _, softmax_real = model(x_real, lambda_p)

            # Forward: Target (sintetico)
            class_out_fake, domain_out_fake, _, softmax_fake = model(x_fake, lambda_p)
            ...
            # Domain loss con Entropy Conditioning (CDAN+E)
            # criterion_domain ha reduction='none' → (B, 1) per campione
            domain_loss_real = criterion_domain(domain_out_real, d_real)  # (B, 1)
...
def evaluate_cdan_synth(model, ckpt_path, test_loader, class_names, device,
                        tag="CDAN_Synth", out_dir=None):
    """
