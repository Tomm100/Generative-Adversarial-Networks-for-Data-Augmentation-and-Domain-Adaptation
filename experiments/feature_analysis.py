...
# Root del repo nel path: consente l'esecuzione da experiments/ (python experiments/feature_analysis.py)
import sys
...
USE_EXTRACTOR = "resnet"  # ["resnet", "dann"]
...
class ResNetFeatureExtractor(nn.Module):
    ...
    def __init__(self, weights_path, device):
        ...
        if os.path.exists(weights_path):
            ...
            print(f"  Pesi ResNet non trovati. Uso pesi ImageNet (baseline).")
...
class DANNFeatureExtractor(nn.Module):
    ...
    def __init__(self, weights_path, device):
        ...
        if os.path.exists(weights_path):
            ...
            print(f"  Pesi DANN non trovati. Uso pesi ImageNet (baseline).")
...
def load_feature_extractor(device):
    ...
    if USE_EXTRACTOR == "resnet":
        ...
        raise ValueError(f"USE_EXTRACTOR non valido: '{USE_EXTRACTOR}'. Usa 'resnet', 'dann'.")
...
def main():
    global USE_EXTRACTOR, RESNET_WEIGHTS_PATH, DANN_WEIGHTS_PATH, GAN_WEIGHTS_PATH, NUM_SAMPLES_PER_CLASS
    parser = argparse.ArgumentParser(
        description="Feature space analysis (PCA/t-SNE/UMAP): reali vs sintetici.")
    parser.add_argument("--extractor", choices=["resnet", "dann"], default=USE_EXTRACTOR,
                        help="Feature extractor da usare.")
    parser.add_argument("--resnet-ckpt", default=RESNET_WEIGHTS_PATH,
                        help="Pesi ResNet (usati se --extractor resnet).")
    parser.add_argument("--dann-ckpt", default=DANN_WEIGHTS_PATH,
                        help="Pesi DANN (usati se --extractor dann).")
    parser.add_argument("--gan-ckpt", default=GAN_WEIGHTS_PATH,
                        help="Pesi del generatore GAN (sintetici on-the-fly).")
    parser.add_argument("--num-samples", type=int, default=NUM_SAMPLES_PER_CLASS,
                        help="Campioni per gruppo (Real N / Real P / Fake N).")
    args = parser.parse_args()
    USE_EXTRACTOR         = args.extractor
    RESNET_WEIGHTS_PATH   = args.resnet_ckpt
    DANN_WEIGHTS_PATH     = args.dann_ckpt
    GAN_WEIGHTS_PATH      = args.gan_ckpt
