...
# Root del repo nel path: consente l'esecuzione da evaluation/ (python evaluation/eval_roc_pr.py)
import sys
...
def main():
    global RESNET_BASELINE_CKPT, RESNET_FINALE_CKPT, FINALE_LABEL
    parser = argparse.ArgumentParser(description="Curve ROC/PR: Baseline vs Finale (solo inferenza).")
    parser.add_argument("--baseline-ckpt", default=RESNET_BASELINE_CKPT,
                        help="Checkpoint ResNet baseline (.pth).")
    parser.add_argument("--finale-ckpt", default=RESNET_FINALE_CKPT,
                        help="Checkpoint ResNet finale/augmentato (.pth).")
    parser.add_argument("--label", default=FINALE_LABEL, help="Etichetta del modello finale.")
    args = parser.parse_args()
    RESNET_BASELINE_CKPT = args.baseline_ckpt
    RESNET_FINALE_CKPT   = args.finale_ckpt
    FINALE_LABEL         = args.label
